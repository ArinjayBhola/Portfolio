...
export function TopLocations({
  ...
  return (
    <div className="lg:col-span-4 bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm flex flex-col items-center justify-between text-center group">
      <div className="space-y-2">
        <div className="w-16 h-16 bg-indigo-50 rounded-[2rem] flex items-center justify-center border border-indigo-100 shadow-inner mx-auto mb-6 transform group-hover:scale-110 transition-all duration-700">
        ...
        {countryData.length > 0 ? countryData.map((item, idx: number) => (
          <div key={item.name} className="space-y-2">
            <div className="flex justify-between items-end">
              <span className="text-[11px] font-semibold text-slate-600 uppercase tracking-widest">{item.name}</span>
      ...
      <div className="mt-10 pt-8 border-t border-slate-100 w-full flex justify-between items-center">
        <div className="text-left">
          ...
          <div className="flex gap-1 mt-1.5">
            {[...Array(5)].map((_, i) => (
              <div key={i} className={`h-1 w-3 rounded-full ${totalVisitorsCount > 0 && i < 2 ? 'bg-indigo-400' : 'bg-slate-200'}`} />
            ))}
          </div>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold text-slate-900 tabular-nums leading-none tracking-tight">{totalVisitorsCount}</p>
